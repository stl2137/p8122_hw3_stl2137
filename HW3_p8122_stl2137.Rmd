...
min(ps[x$PracticeType==1])
ps[which(x$PracticeType_bin==0)]<= min(ps[x$PracticeType==1])
length(ps[which(x$PracticeType_bin==0)]<= min(ps[x$PracticeType==1]))
#eliminate treated for whom the P(A=1|C) is greater that the max(P(A=1|C)) found in the control group
max(ps[x$PracticeType==0])
ps[which(x$PracticeType_bin==1)]>= max(ps[x$PracticeType==0])

length(ps[which(x$PracticeType_bin==0)]<= min(ps[x$PracticeType==0]))

data = x[ps>=min(ps[x$PracticeType==1]) & ps <= max(ps[x$PracticeType==0]),] 
dim(x)
dim(data)
```
